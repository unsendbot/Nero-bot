...
module.exports = {
	config: {
		...
		guide: {
			...
			ar: "{pn} [فارغ | <رقم الصفحة> | <اسم الامر>]"
				+ "\n   {pn} <اسم الامر> [-u | الاستعمال | -g | guide]: يريك طريقة عمل الامر فقط"
				+ "\n   {pn} <اسم الامر> [-i | معلومات]: يريك معلومات الامر فقط"
				+ "\n   {pn} <الصلاحية> [-r | role]: يريك صلاحية الامر فقط"
				+ "\n   {pn} <اسم الامر> [-a | alias | اسماء]: يريك اسماء الامر"
		},
		priority: 1
	},
	...
	langs: {
		...
		ar: {
			help: "╭─────────────⭓"
				...
				+ "\n├─────⭔"
				+ "\n│ الصفحة [ %2/%3 ]"
				+ "\n│ حاليا البوت فيه %4 امر صالح للاستعمال"
				+ "\n│ » اكتب %5اوامر <رقم الصفحة> لعرض الصفحات الأخرى"
				+ "\n│ » أكتب %5شرح مع اسم الامر لرؤية التفاصيل "
				+ "\n├────────⭔"
				+ "\n│ %6"
				+ "\n╰─────────────⭓",
			help2: "%1├───────⭔"
				+ "\n│ » البوت فيه  %2 صالح للاستعمال"
				+ "\n│ » أكتب %3أوامر <إسم الأمر> لرؤية شرح الأمر"
				+ "\n│ %4"
				+ "\n╰─────────────⭓",
			commandNotFound: "أمر \"%1\" غير موجود في البوت",
			getInfoCommand: "╭── NAME ────⭓"
				+ "\n│ %1"
				+ "\n├── INFO"
				+ "\n│ Description: %2"
			...
			roleText2: "2 (أدمن البوت)",
			roleText0setRole: "0 (صلاحية كل المستخدمين)",
			roleText1setRole: "1 (صلاحية أدمن المجموعة)",
	...
	onStart: async function ({ message, args, event, threadsData, getLang, role, globalData }) {
		...
		if (langCode == "ar") {
				if (!args[0]) {
						return message.reply("أكتب إسم الأمر.");
				}
			...
			if (sortHelp == "name") {
				...
				for (const [name, value] of commands) {
					if (value.config.role > 1 && role < value.config.role)
						continue;
					if (value.config.hided && role < 2)       continue;
					let describe = name;
					let description;
