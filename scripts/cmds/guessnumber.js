...
module.exports = {
	config: {
		...
		guide: {
			...
			en: "  {pn} [4 | 5 | 6] [single | multi]: create a new game, with:"
                ...
                + "\n    {pn} 4 أحادي"
                + "\n"
                + "\n   كيفية اللعب: يرد اللاعب على رسالة البوت بالقواعد التالية:"
                + "\n   لديك " + rows.map(item => `${item.row} مرات (${item.col} أرقام)`).join(", ") + "."
                + "\n   بعد كل تخمين، ستحصل على تلميحات إضافية لعدد الأرقام الصحيحة (معروضة على اليسار) وعدد الأرقام الصحيحة (معروضة على اليمين)."
                + "\n   ملاحظة: الرقم مكون من أرقام من 0 إلى 9، كل رقم يظهر مرة واحدة فقط ويمكن أن يبدأ الرقم بـ 0."
                + "\n\n   {pn} ترتيب <صفحة>: عرض الترتيب."
                + "\n   {pn} معلومات [<معرف المستخدم> | <@علامة> | <رد> | <فارغ>]: عرض معلومات الترتيب الخاصة بك أو بالآخرين."
                + "\n   {pn} إعادة تعيين: إعادة تعيين الترتيب (فقط مشرف البوت)."
		}
	},
	...
	langs: {
		...
		ar: {
	        ...
	        gameNote: "📄 | ملاحظة:\nالرقم مكون من أرقام من 0 إلى 9، كل رقم يظهر مرة واحدة فقط ويمكن أن يبدأ الرقم بـ 0.",
	       replyToPlayGame: "🎮 | رد على الرسالة أدناه بالصورة التي تحتوي على %1 أرقام التي تخمنها للعب اللعبة.",
	       invalidNumbers: "⚠️ | يرجى إدخال %1 أرقام التي تريد تخمينها",
	       win: "🎉 | تهانينا، لقد خمنت الرقم %1 بعد %2 تخمينات وحصلت على %3 نقاط إضافية.",
	...
	onStart: async function ({ message, event, getLang, commandName, args, globalData, usersData, role }) {
		...
		else if (args[0] == "معلومات" || args[0] == "info") {
			const rankGuessNumber = await globalData.get("rankGuessNumber", "data", []);
			let targetID;
