module.exports = {
	config: {
		...
		guide: {
			...
			ar: "   {pn} [thread | user]: تحديث المجموعة او المستخدم"
				+ "\n   {pn} group <threadID>: تحديث المجموعة "
				+ "\n\n   {pn} user: تحديث المستخدم"
				+ "\n   {pn} user [<userID> | @تاغ]: صاحب التاغ او الايدي يتحدث معلوماته"
		}
	},
	...
	onStart: async function ({ args, threadsData, message, event, usersData, getLang }) {
		if (args[0] == "group" || args[0] == "thread" || args[0] == "المجموعة") {
